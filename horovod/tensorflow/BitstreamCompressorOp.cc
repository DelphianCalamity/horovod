...
.Input("compressed_tensor: uint32")
...
public:
    ...
    void Compute(OpKernelContext *context) override {
        ...
        auto compressed_tensor_flat = compressed_tensor.flat<uint32_t>();
        const size_t compressed_tensor_size = compressed_tensor_flat.size();
        IntegerCODEC &codec = *CODECFactory::getFromName("vbyte");   // Pick a CODEC

        std::vector<uint32> bitcompressed_output(compressed_tensor_size + 1024);
        size_t bitcompressed_size = bitcompressed_output.size();
        codec.encodeArray(compressed_tensor_flat.data(), compressed_tensor_size, bitcompressed_output.data(), bitcompressed_size);
        ...
        std::cout << "You are using " << 32.0 * static_cast<double>(bitcompressed_output.size()) /
                     static_cast<double>(compressed_tensor_flat.size()) << " bits per integer. " << std::endl;
