...
def compress(tensor, params):
	...
	break_points, sizes = find_breaks(values, params['num_of_segments'], N)
	print(sizes)
	coefficients = []
	for i in range(params['num_of_segments']):
		x = tf.reshape(tf.cast(tf.range(0, sizes[i]), tf.int64), [1, sizes[i]])
		X = GetInputMatrix_Polynomial(params['polynomial_degree'], x)
		y = tf.reshape(values[break_points[i]: break_points[i + 1]], [sizes[i], 1])
		coefficients += [LeastSquares(X, tf.cast(y, tf.float64))]
	...
	logger = logger(tensor_flatten, tf.cast(coefficients, tf.float64),
					0,
