...
class Bloom_Filter_Compressor(Compressor):
    """"""
    @staticmethod
    def bloom_configuration(self, k, fpr):
...
#         if params["hash_functions_number"] is not None:
#             params['k'] = params['hash_functions_number']
#         if params["fpr"] is not None:   # Given FPR compute M and H
#             m = (k * abs(math.log(params["fpr"]))) / (math.pow(math.log(2), 2))
#             params['m'] = m
#
#         # Give bloom size in number of bytes bloom size must be a multiple of 8
...
#         compressed_tensor = bloom_compressor(tensor_flatten, indices,
#                                              tf.train.get_or_create_global_step(),
#                                              hash_num=params['k'],
#                                              bloom_size=params['m'],
#                                              logfile_suffix=params['logfile_suffix'],
...
#
#
# class Fp_Aware_Bloom_Filter_Compressor_Conflict_sets(Compressor):
#     """"""
#
#     @staticmethod
#     def compress(tensor, params):
...
#         filename = resource_loader.get_path_to_datafile('mpi_lib.so')
#         library = load_library.load_op_library(filename)
#         bloom_compressor = library.fp_aware_bloom_compressor_conflict_sets
#
#         # For debugging
#         tensor_flatten = tf.bitcast(tensor_flatten, tf.int32)
#         compressed_tensor = bloom_compressor(tensor_flatten, indices,
...
class Compression(object):
    ...
    # fp_aware_bloom = Fp_Aware_Bloom_Filter_Compressor
    ...
    # stacked_bloom_conflict_sets = Stacked_Bloom_Filter_Compressor_Conflict_Sets